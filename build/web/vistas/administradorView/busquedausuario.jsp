<%-- 
    ...
    Created on : 2/10/2022, 1:21:49 a. m.
    Author     : reygr
--%>
		...
		<section class="full-box page-content">
			...
			<div class="container-fluid">
				<ul class="full-box list-unstyled page-nav-tabs">
					...
					<li>
						<a class="active" href="./busquedausuario.jsp"><i class="fas fa-search fa-fw"></i> &nbsp; BUSCAR USUARIO</a>
					</li>
				</ul>	
			</div>
							...
							<tr class="text-center roboto-medium">
								...
								<th>ELIMINAR</th>
                                                ...
                                                try {
                                                        ...
                                                        rs=sta.executeQuery("SELECT idusuario,dni,nombres,apellidos,celular,correo FROM usuario"); 
                                                        buscarnombre = request.getParameter("busqueda");
                                                        if (buscarnombre == null) {                                                                                                                            
                                                            rs=sta.executeQuery("SELECT idusuario,dni,nombres,apellidos,celular,correo FROM usuario"); 
                                                            System.out.println("buscarnombre = null: " +buscarnombre );
                                                        }else{
                                                            rs=sta.executeQuery("SELECT idusuario,dni,nombres,apellidos,celular,correo FROM usuario WHERE nombres LIKE"+"'%" + buscarnombre+"%'");   
                                                            System.out.println("buscarnombre != null: " +buscarnombre );
                                                        }
                                                        while(rs.next()){
                                                            %>
                                                            <tr class="text-center">
                                                                <form action="/DentoSmile/EliminarUsuarioController" method="post">
                                                                    <th style="display: none;"><%=rs.getString(1)%> <input type="hidden" name="idusuario" value="<%=rs.getString(1)%>"></th>
                                                                    <th><%=rs.getString(2)%> <input type="hidden" name="usuario_dni" value="<%=rs.getString(2)%>"></th>
                                                                    <th><%=rs.getString(3)%> <input type="hidden" name="usuario_nombres" value="<%=rs.getString(3)%>"></th>
                                                                    <th><%=rs.getString(4)%> <input type="hidden" name="usuario_apellidos" value="<%=rs.getString(4)%>"></th>
                                                                    <th><%=rs.getString(5)%> <input type="hidden" name="usuario_celular" value="<%=rs.getString(5)%>"></th>
                                                                    <th><%=rs.getString(6)%> <input type="hidden" name="usuario_email" value="<%=rs.getString(6)%>"></th>
                                                                    <td>
                                                                            <a href="./actualizausuario.jsp?idusuario=<%=rs.getString(1)%>" class="btn btn-success">
